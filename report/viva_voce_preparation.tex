...
\subsection{Spark Performance Results}

\textbf{Actual Measured Performance (SPP 100 events/sec)}:
...
Unique Windows Processed & 32 windows \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Spark Performance Metrics - Actual Results}
\end{table}
...
{"campaign_id":"c3956cd9-ea10-4033-aeea-290269e83fa4",
 ...
 "views":4,"clicks":1,"ctr":0.25,
 "result_generation_time_ms":1763607807000,
 "min_insertion_time_ms":1763607799686}
\end{verbatim}

Latency calculation: $1763607807000 - 1763607799686 = 7314$ ms $\approx$ 7.3 seconds

\textbf{Analysis}:
\begin{itemize}
    \item Mean latency of 16.05 seconds matches theoretical expectation (10s window + 5s watermark + processing)
    \item Low standard deviation (1.67s) indicates consistent performance
    \item P99 latency (16.994s) very close to mean, showing minimal outliers
    \item Minimum latency (7.04s) likely represents first incomplete window
\end{itemize}

\subsection{Flink Implementation Status}

Flink implementation is complete but requires specific startup sequence:
\begin{enumerate}
    \item Start data generator FIRST
    \item Wait 30 seconds for topic to populate
    \item THEN start Flink job
\end{enumerate}

\textbf{Issue}: Flink fails with \texttt{TimeoutException} when connecting to empty Kafka topic. This is a known limitation requiring data to exist before Flink starts consuming.
%═══════════════════════════════════════════════════════════════════
...
\subsection{Challenge 5: Flink Empty Topic Timeout}

\subsubsection{Problem}
Flink fails when starting before data exists in topic:
\begin{verbatim}
...
\subsubsection{Root Cause}
Flink's Kafka consumer cannot determine starting offset when topic is empty or newly created.

\subsubsection{Solution - CRITICAL STARTUP SEQUENCE}
\begin{enumerate}
    \item Start data generator FIRST
    \item Wait 30 seconds for data to populate topic
    \item THEN start Flink job (while generator is still running)
    \item Let both run together for test duration
\end{enumerate}

\textbf{Key Lesson}: Always ensure Kafka topics have data before starting Flink consumers.
